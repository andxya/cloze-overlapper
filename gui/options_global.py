...
from ..config import config, get_syntax_example
from ..consts import *
...
class OlcOptionsGlobal(QDialog):
    ...
    def setupSyntaxConnections(self):
        """Connect syntax fields to update the example label"""
        self.f.le_wrapper_start.textChanged.connect(self.updateSyntaxExample)
        self.f.le_wrapper_end.textChanged.connect(self.updateSyntaxExample)
        self.f.le_prefix.textChanged.connect(self.updateSyntaxExample)

    def updateSyntaxExample(self):
        """Update the syntax example label based on current field values"""
        ws = self.f.le_wrapper_start.text()
        we = self.f.le_wrapper_end.text()
        prefix = self.f.le_prefix.text()
        example = "{ws}{p}1::text{we}".format(ws=ws, p=prefix, we=we)
        self.f.label_syntax_example.setText("Example: " + example)
    ...
    def setupValues(self, values):
        ...
        self.f.le_wrapper_start.setText(syntax.get("wrapper_start", "{{"))
        self.f.le_wrapper_end.setText(syntax.get("wrapper_end", "}}"))
        self.f.le_prefix.setText(syntax.get("prefix", "o"))
        self.updateSyntaxExample()
    ...
    def onAccept(self):
        ...
        # Save syntax configuration
        config["synced"]["syntax"] = {
            "wrapper_start": self.f.le_wrapper_start.text(),
            "wrapper_end": self.f.le_wrapper_end.text(),
            "prefix": self.f.le_prefix.text()
        }
