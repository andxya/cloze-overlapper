...
# Try to import gui_hooks for modern Anki (2.1.20+)
try:
...
from .config import config, get_cloze_regex
from .utils import showTT


def get_oc_cloze_re():
    """Get the regex pattern to find overlapping cloze numbers"""
    ...
    return ws + prefix + r"(\d+)::"


def get_oc_wrap_strings():
    """Get the wrap strings for overlapping cloze insertion"""
    syntax = config["synced"].get("syntax", {
        "wrapper_start": "{{",
        "wrapper_end": "}}",
        "prefix": "o"
    })
    # Returns (wrap_left, wrap_right) with %d placeholder for number
    wrap_left = syntax["wrapper_start"] + syntax["prefix"] + "%d::"
    wrap_right = syntax["wrapper_end"]
    return wrap_left, wrap_right


def get_oc_full_cloze_re():
    """Get the full regex pattern for removing clozes"""
    syntax = config["synced"].get("syntax", {
        "wrapper_start": "{{",
        "wrapper_end": "}}",
        "prefix": "o"
...
olc_hotkey_regular_cloze = "Ctrl+Shift+R"  # Regular Anki cloze
olc_hotkey_append_regular_cloze = "Ctrl+Alt+Shift+R"  # Append regular Anki cloze
...
def onCloze(self, reuse: bool):
    ...
    # Use configurable syntax for overlapping clozes
    oc_re = get_oc_cloze_re()
    wrap_left, wrap_right = get_oc_wrap_strings()
    return insert_cloze_helper(self, oc_re, reuse, wrap_left, wrap_right)
...
def onInsertRegularCloze(self):
    """Insert regular Anki cloze {{c1::}} on overlapping note type"""
    return insert_cloze_helper(self, ANKI_CLOZE_RE, False, "{{c%d::", "}}")


def onAppendRegularCloze(self):
    """Append regular Anki cloze {{c1::}} on overlapping note type"""
    return insert_cloze_helper(self, ANKI_CLOZE_RE, True, "{{c%d::", "}}")
...
to a cloze type first, via Edit>Change Note Type.""")
    ...
    if checkModel(model, fields=False, notify=False):
        cloze_re = get_oc_cloze_re()
        # Get wrap strings without the %d placeholder
        syntax = config["synced"].get("syntax", {
            "wrapper_start": "{{",
            "wrapper_end": "}}",
            "prefix": "o"
        })
        wrap_pre = syntax["wrapper_start"] + syntax["prefix"]
        wrap_post = syntax["wrapper_end"]
    else:
...
def onAddCards(self, _old):
    ...
    if ANKI20:
        ...
        # For Anki 2.1+, ensure note fields are synced from editor
        # The note object should already be updated by the time _add_current_note is called
        # but let's make sure we have the latest data
    ...
    # Reload the note in the editor to show generated fields
    refreshEditor(editor)
...
def onAddCardsGuiHook(addcards: "AddCards", note) -> None:
    """gui_hooks callback - generate overlapping clozes before adding (modern Anki)

    This is called by add_cards_will_add_note hook which fires right before
    the note is added to the collection.
    """
    if not note or not checkModel(note.model(), notify=False):
        return
    ...
    # Note is modified in place, changes will be saved when the note is added
    if total:
        showTT("Info", "Generated %d overlapping cloze cards" % total, period=1000)
...
tooltip_regular_cloze = "Regular Anki cloze ({})".format(
    olc_hotkey_regular_cloze)
tooltip_append_regular_cloze = "Append regular Anki cloze ({})".format(
    olc_hotkey_append_regular_cloze)
...
def setupAdditionalHotkeys(editor):
    ...
    # Regular Anki cloze hotkeys
    regular_cloze_cut = QShortcut(QKeySequence(olc_hotkey_regular_cloze), editor.widget)
    regular_cloze_cut.activated.connect(lambda: onInsertRegularCloze(editor))
    append_regular_cloze_cut = QShortcut(QKeySequence(olc_hotkey_append_regular_cloze), editor.widget)
    append_regular_cloze_cut.activated.connect(lambda: onAppendRegularCloze(editor))
...
def initializeEditor():
    ...
    Editor.onInsertRegularCloze = onInsertRegularCloze
    Editor.onAppendRegularCloze = onAppendRegularCloze
    if ANKI20:
    ...
    if ANKI20:
        ...
        # Use gui_hooks for modern Anki (most reliable method)
        if HAS_GUI_HOOKS and hasattr(gui_hooks, 'add_cards_will_add_note'):
            gui_hooks.add_cards_will_add_note.append(onAddCardsGuiHook)
        else:
            # Fallback: Hook the correct method based on Anki version
            # In Anki 2.1.55+, _addCards was renamed to _add_current_note
            if hasattr(AddCards, '_add_current_note'):
                AddCards._add_current_note = wrap(AddCards._add_current_note, onAddCards, "around")
            elif hasattr(AddCards, '_addCards'):
                AddCards._addCards = wrap(AddCards._addCards, onAddCards, "around")

        if hasattr(EditCurrent, '_saveAndClose'):
